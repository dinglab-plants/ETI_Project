...
cngc_list$significance[cngc_list$adj.pval <= 0.01] = "*"

cngc_barplot = ggplot(fig_table_long, mapping = aes(x = Gene_Name, y = Log2FC, fill = Treatment)) +
  geom_bar(stat = "identity", position = "dodge", width = 0.75) + 
  scale_fill_manual(values = c("#E69F00", "#0072B2", "#12A27B"),
                    name = "Treatment", labels = c("PTI", "ETI", "PTI + ETI")) +
  scale_x_discrete(guide = guide_axis(n.dodge = 2)) +
  geom_hline(yintercept=0) +
  geom_text(data = fig_table_long[fig_table_long$Log2FC > 0,], mapping = aes(x = Gene_Name, y = Log2FC, label = significance), position = position_dodge(width = 0.75), size = 5) +
   geom_text(data = fig_table_long[fig_table_long$Log2FC < 0 | fig_table_long$significance == "",], mapping = aes(x = Gene_Name, y = Log2FC - 0.3, label = significance), position = position_dodge2(width = 0.75), size = 5) +
  dlab_theme 

ggsave("cngc_barplot.pdf", plot = cngc_barplot, units = "in", width = 8,
  height = 4.5)
